...
// This object maps element IDs to variables for easy access.
const elements = {
    ...
    dashboardExamGrid: document.getElementById('dashboard-exam-grid'),
    examSelectionContainer: document.getElementById('exam-selection-container'),
...
export function showView(viewId) {
    [elements.landingPageView, elements.dashboardView, elements.examView].forEach(view => {
...
export async function loadDynamicContent() {
    // Corrected paths to be absolute from the server root
    const [features, testimonials, faqs] = await Promise.all([
        fetchData('/client/data/features.json'),
        fetchData('/client/data/testimonials.json'),
        fetchData('/client/data/faqs.json')
    ]);

    if (elements.featuresContainer) {
        elements.featuresContainer.innerHTML = features.map(f => `
            <div class="feature-card p-8 rounded-xl">
                <div class="feature-icon-bg">${f.icon}</div>
    ...
    if (elements.testimonialsContainer) {
        elements.testimonialsContainer.innerHTML = testimonials.map(t => `
            <div class="testimonial-card p-8 rounded-xl relative">
                <span class="absolute top-4 left-6 text-7xl brand-yellow opacity-10 font-serif">“</span>
                <p class="text-lg text-gray-200 z-10 relative">${t.quote}</p>
    ...
    if (elements.faqAccordion) {
        elements.faqAccordion.innerHTML = faqs.map(f => `
            <div class="faq-item">
...
// --- Stats UI ---
export function displayStats(stats) { if (elements.totalVisits && elements.uniqueUsers) { elements.totalVisits.textContent = stats.totalVisits.toLocaleString(); elements.uniqueUsers.textContent = stats.uniqueUsers.toLocaleString(); } }

// --- Dashboard & Exam Selection UI ---
export function renderDashboard(examTree) { if (elements.dashboardExamGrid) { elements.dashboardExamGrid.innerHTML = Object.keys(examTree).map(key => { const exam = examTree[key]; return `<div class="dashboard-exam-card" data-action="select-exam-category" data-key="${key}"><h3>${exam.title}</h3><p>${exam.description}</p></div>`; }).join(''); } }
export function renderExamSelection(title, categories) { elements.selectionTitle.textContent = title; elements.selectionGrid.innerHTML = Object.keys(categories).map(key => { const category = categories[key]; return `<div class="exam-category-item" data-action="select-exam-category" data-key="${key}"><h3>${category.title}</h3></div>`; }).join(''); elements.examSelectionContainer.classList.remove('hidden'); elements.examInProgressSection.classList.add('hidden'); elements.resultsSection.classList.add('hidden'); }
export function showExamInProgress() { elements.examSelectionContainer.classList.add('hidden'); elements.examInProgressSection.classList.remove('hidden'); elements.resultsSection.classList.add('hidden'); }
export function resetExamUI() { elements.examSelectionContainer.classList.add('hidden'); elements.examInProgressSection.classList.add('hidden'); elements.resultsSection.classList.add('hidden'); }

// --- Theme and Fullscreen UI ---
